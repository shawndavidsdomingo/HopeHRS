...
// ─────────────────────────────────────────────────────────────────────────────
// HeadcountByDeptPage:
//   - Bar chart showing active employee count per department
//   - Table below the chart with deptCode, deptName, activeHeadcount
//   - Data from getHeadcountByDept() via reportsService.js
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div className="bg-white border border-slate-200 p-6 mb-8">
      ...
      <div className="space-y-3">
        {data.map((row) => (
          <div key={row.deptcode} className="flex items-center gap-4">
            <span className="text-xs font-mono text-slate-400 w-16 shrink-0 text-right">
              {row.deptcode}
            </span>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.18em]">Dept Code</th>
              <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.18em]">Department Name</th>
              <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.18em] text-right">Active Headcount</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              data.map((row) => (
                <tr key={row.deptcode} className="border-b border-slate-100 last:border-0 hover:bg-indigo-50/40 transition-colors duration-75">
                  <td className="px-6 py-4 font-mono text-xs text-slate-400">{row.deptcode}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-slate-800">{row.deptname}</td>
