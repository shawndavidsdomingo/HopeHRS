...
// ─────────────────────────────────────────────────────────────────────────────
// JobListPage
//   Columns : jobCode, jobDesc, record_status (ADMIN/SUPERADMIN only)
//   Add     : gated by JOB_ADD
//   Edit    : gated by JOB_EDIT (per-row button)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function Jobs() {
  ...
  const [editTarget, setEditTarget] = useState(null); // job row being edited
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                {rights?.JOB_EDIT === 1 && <Th align="right">Actions</Th>}
              </tr>
            </thead>
            <tbody>
              {jobs.map((job, idx) => (
      ...
      {/* ── Modals ── */}
      {showAdd && (
        <AddJobModal onClose={() => setShowAdd(false)} onSuccess={load} />
      )}
      {editTarget && (
        <EditJobModal job={editTarget} onClose={() => setEditTarget(null)} onSuccess={load} />
      )}
    </div>
  );
}
